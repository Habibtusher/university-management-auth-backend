...
const getAcademicFacultyFromDb = (filters, pagination) => __awaiter(void 0, void 0, void 0, function* () {
    const { searchTerm } = filters, filtersData = __rest(filters, ["searchTerm"]);
    const andCondition = [];
    if (searchTerm) {
        andCondition.push({
            $or: academicFaculty_constant_1.academicFacultysearchFields.map(field => ({
                [field]: {
                    $regex: searchTerm,
                    $options: 'i',
                },
    ...
    const result = yield academicFaculty_model_1.default.find(whereCondition)
        .sort(sortOpts)
        .skip(skip)
        .limit(limit);
